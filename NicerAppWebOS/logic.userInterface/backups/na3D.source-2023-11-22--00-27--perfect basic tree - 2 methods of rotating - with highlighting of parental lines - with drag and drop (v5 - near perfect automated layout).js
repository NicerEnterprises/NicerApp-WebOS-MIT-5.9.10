...
export class na3D_fileBrowser {
    ...
    onresize_do(t, callback) {
        ...
        for (var i=0; i<t.items.length; i++) {
            ...
            if (it.parent && !pox[it.parent.idx]) pox[it.parent.idx] = Math.abs(Math.random() * 400);
            if (it.parent && !poy[it.parent.idx]) poy[it.parent.idx] = Math.abs(Math.random() * 400);
            if (it.parent && !poz[it.parent.idx]) poz[it.parent.idx] = Math.abs(Math.random() * 400);
...
//debugger;
            ...
            if (it.model && p && p.model) {
                ...
                m = 200, n = 1.25, o = 100,
                plc = p.leftRight === 0 ? 0.5 : p.leftRight,
                plr = p.upDown === 0 ? 0.5 : p.upDown;
                it.model.position.x = Math.round(
                    ...
                    //+ (it.level > 6 ? (pModifierC * p.columnOffsetValue*(it.column *m)) : (it.column*m))
                    //+ (it.level > 4 ? (pModifierC * p.columnOffsetValue * p.column * m * n) : 0)
                    //+ (it.level > 4 ? (pModifierC * p.columnOffsetValue * p.column * n * o) : 0)
                    ////+ (it.level > 4 ? (p.columnOffsetValue* rndx) : 0)

                    //+ (it.level > 0 ? (p.columnOffsetValue*(it.column *m)) : (it.column*m))
                    //+ (it.level > 4 ? (-1 * p.columnOffsetValue * p.column * m * n) : 0)
                    //+ (it.level > 4 ? (-1 * p.columnOffsetValue * p.column * n * o) : 0)

                    ////+ (it.level > 4 ? (it.leftRight*(it.column *m)) : (it.column*m))
                    //+ (it.level > 4 ? (p.leftRight*(it.column *m)) : (it.column*m))
                    //+ (it.level > 4 ? (p.leftRight * p.column * m * n) : 0)
                    //+ (it.level > 4 ? ( p.leftRight * p.column * n * o) : 0)

                    + (it.level > 4 ? (plc * (it.column *m)) : (it.column*m))
                    + (it.level > 4 ? (plc * p.column * m * n) : 0)
                    + (it.level > 4 ? (plc * p.column * n * o) : 0)
                );
                it.model.position.y = Math.round(
                    ...
                    //+ (it.level > 6 ? (pModifierR * p.rowOffsetValue* (it.row *m)) : (it.row*m))
                    //+ (it.level >   4 ? (pModifierR * p.rowOffsetValue * p.row * m * n) : 0)
                    //+ (it.level >   4 ? (pModifierR * p.rowOffsetValue * p.row * n * o) : 0)
                    ////+ (it.level > 4 ? (p.rowOffsetValue * rndy) : 0)

                    ////+ (it.level > 4 ? ( it.upDown* (it.row *m)) : (it.row*m))
                    //+ (it.level > 4 ? ( p.upDown* (it.row *m)) : (it.row*m))
                    //+ (it.level >   4 ? ( p.upDown* p.row * m * n) : 0)
                    //+ (it.level >   4 ? ( p.upDown* p.row * n * o) : 0)

                    + (it.level > 4 ? ( plr* (it.row *m)) : (it.row*m))
                    + (it.level >   4 ? ( plr* p.row * m * n) : 0)
                    + (it.level >   4 ? ( plr* p.row * n * o) : 0)
