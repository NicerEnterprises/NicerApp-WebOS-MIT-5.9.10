seductiveapps.hms = seductiveapps.jsonViewer = {
	about : {
		...
		copyright : '(c) (r) 2010-2013 by [the owner of seductiveapps.com] <seductiveapps@gmail.com>',
	...
	options: {
		...
		authorsDefaults: {
			...
			themeChoices: {
				hmDefault: 'saColorgradientScheme_BlueToNavyBG_white',
	...
	ui: {
		...
		expandMore: function (expandCmd) {
			...
			if (expandCmd.ids[k].levels > 0) {
				...
				if (!ph || doo) {
                    if (doo) debugger;
					sa.hms.tools.printLevel (el, expandCmd.ids[k].levels, expandCmd);
				} else {
					sa.hms.ui.showHide (el, doo);
				}
			};

			//sa.hms.tools.fireEvent (sa.hms.options.thisCmd,'onResized',{loc:'expandMore'});
            sa.hms.tools.onResize (sa.hms.options.thisCmd.id)
			return true;
		},
	...
	tools: {
		...
		waitForParsingDone : function (cmd) {
			...
			sa.hms.options.current = jQuery.extend(
			...
			for (cn in sa.hms.options.contributors) {
				sa.hms.options.current.themes = jQuery.extend(
				sa.hms.options.current.themes, sa.hms.options.contributors[cn].themes);
			};

			sa.hms.options.current.theme = jQuery.extend(
			...
			var htmlLegend = 
				...
				'<table class="hmLegend2"><tr style="width:100%;">' + 
					...
					( cmd.hmd && cmd.hmd.hms && cmd.hmd.hms.topKeys > 0 
						? sa.hms.tools.htmlArrayHeader(cmd.hmd, 'key', true) 
	...
	//		debugger;
		...
		onResize : function (cmdID) {
            if (sa.apps.loaded.jsonViewer) sa.apps.loaded.jsonViewer.settings.loadedIn['#siteContent'].onresize({callHM:false});
			...
			if (
				...
				if (cmd && cmd.externalScrollpaneID) {
					...
					sa.sp.containerSizeChanged (jQuery('#'+cmd.externalScrollpaneID)[0], false);
				}
			};
		},
		...
		printLevel: function (scheduledToPopulate_element, levels, expandCmd) {
            ...
            if (!pvCmd.cmd) pvCmd.cmd = sa.hms.settings.cs.cmds['jsonViewer'];
									...
									?'<tr><td colspan="2">'
										+'<span class="hmArrayKeyLegend">' + 
											(!pvCmd.theme || pvCmd.theme.showArrayStats 
												? sa.hms.tools.htmlArrayHeader(item.d, 'keys', isRoot) + '<br/>' 
												: ''
											) + 
											'</span>'
									+'</td></tr>'
		...
		htmlArrayHeader: function (v, type, isRoot) {
			...
			if (typeof v == 'object' && typeof v.hms == 'object') {
				s =
					...
					(v.hms.depth>1?', ' + sa.hms.tools.htmlArrayExpandList(v, type, id, isRoot):'')+
					' ]';
			};
			return s;
		},
		...
		htmlArrayExpandList : function (v, type, id, isRoot){
			...
			for (var i=2; i<=v.hms.depth; i++) {
				html += '<a href="#" title="Expand '+i+' levels" class="hmNavE" onclick="return sa.hms.ui.expand(\''+id+'\', '+i+', \''+type+'\', '+(isRoot?'true':'false')+');">'+i+'</a> ';
			}
			return html;
		},
