...
global $naWebOS;
    ...
    foreach ($call->body->docs as $docIdx => $doc) {
        ...
        echo '<h2 class="logEntry" onclick="naLog.onclick_logEntry_detail(event);"><span class="datetimeAccurate">'.$now2.'</span> <span class="ip">'.$doc->ip.'</span> '.$docA['request']['$_SERVER']['REQUEST_URI'].'</h2>';
        echo '</div>';
/*
        // fetch dataRecord
        $findCommand2 = [
