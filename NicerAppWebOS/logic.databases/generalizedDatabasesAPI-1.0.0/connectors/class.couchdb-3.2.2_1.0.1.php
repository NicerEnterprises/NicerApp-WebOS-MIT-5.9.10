...
class class_NicerAppWebOS_database_API_couchdb_3_2 {
    ...
    //if (php_sapi_name() === 'cli') return 'php_sapi_name()='.php_sapi_name(); // BAD!
        ...
        if (!is_null($cRec)) $naLoginResult = cdb_login ($this->cdb, $cRec, $username); else cdb_login ($this->cdb, null, null);
...
*/
    ...
    public function createDataSet_cms_tree() {
        ...
        */
        ...
        $data = '{ "database" : "'.$dataSetName.'", "_id" : "caa", "id" : "caa", "parent" : "#", "text" : "Groups", "state" : { "opened" : true }, "type" : "naSystemFolder" }';
        if ($do) try { $this->cdb->post($data); } catch (Exception $e) { echo '<pre>'.json_encode(json_decode($data),JSON_PRETTY_PRINT).'</pre>'; echo $e->getMessage(); echo '<br/>'; };

        $do = false; try { $doc = $this->cdb->get('baa'); } catch (Exception $e) { $do = true; };
        $data = '{ "database" : "'.$dataSetName.'", "_id" : "baa", "id" : "baa", "parent" : "#", "text" : "Users", "state" : { "opened" : true }, "type" : "naSystemFolder" }';
        if ($do) try { $this->cdb->post($data); } catch (Exception $e) { echo '<pre>'.json_encode(json_decode($data),JSON_PRETTY_PRINT).'</pre>'; echo $e->getMessage(); echo '<br/>'; };
    ...
    public function createDataSet_cms_tree__user__guest() {
        ...
        $data = '{ "database" : "'.$dataSetName.'", "_id" : "dbb", "id" : "dbb", "parent" : "dba", "url1" : "on", "seo_value" : "frontpage", "pageTitle" : "Front page", "text" : "Front page", "state" : { "selected" : true }, "type" : "naDocument" }';
        if ($do) try { $this->cdb->post($data); } catch (Exception $e) { echo '<pre>'.json_encode(json_decode($data),JSON_PRETTY_PRINT).'</pre>'; echo $e->getMessage(); echo '<br/>'; };
    ...
    public function createDataSet_cms_documents__user__guest() {
        ...
        $do = false; try { $doc = $this->cdb->get('dbb'); } catch (Exception $e) { $do = true; };
        $data = '{ "database" : "'.$dataSetName.'", "_id" : "dbb", "id" : "dbb", "url1" : "on", "seo_value" : "frontpage", "pageTitle" : "Front page", "document" : "<h1>Front page</h1><p>Start editing your front page here.</p>" }';
        if ($do) try { $this->cdb->post($data); } catch (Exception $e) { echo '<pre>'.json_encode(json_decode($data),JSON_PRETTY_PRINT).'</pre>'; echo $e->getMessage(); echo '<br/>'; };
...
//echo '<pre style="background:blue;color:white;">'; var_dump ($_COOKIE);echo '</pre>';
    ...
    public function cms_editDocument () {
        ...
        $seoValue = array_key_exists('seoValue',$_POST) ? $_POST['seoValue'] : $naWebOS->getDataID($_POST['database'], 'seo_value');
        $document = array (
        ...
        try { $call = $cdb->post($documentToPost); } catch (Exception $e) { cdb_error (500, $e, 'Could not add/update record in '.$_POST['database']); exit(); };
